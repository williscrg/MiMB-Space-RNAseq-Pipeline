...
meta_data = data.frame('Run' = c('SRR26332897', 'SRR26332898', 'SRR26332902', 'SRR26332903', 'SRR26332906', 'SRR26332907'))

### Add condition column ###
meta_data$Condition = ifelse(meta_data$Run %in% c('SRR26332906', 'SRR26332902', 'SRR26332897'), 'GC', 'SF')
...
meta_data$url_R1 = sapply(meta_data$Run, function(x) paste0(url_pre, '/', GEOfastq::get_dldir(x), '/', x, '_1.fastq.gz'))
meta_data$url_R2 = sapply(meta_data$Run, function(x) paste0(url_pre, '/', GEOfastq::get_dldir(x), '/', x, '_2.fastq.gz'))

### Download reads ###
read_urls = c(meta_data$url_R1, meta_data$url_R2)
...
runs = meta_data$Run
R1s = paste0('./Data/Reads', '/', meta_data$Run, '_1.fastq.gz')
R2s = paste0('./Data/Reads', '/', meta_data$Run, '_2.fastq.gz')
out_dirs = paste0('./Outputs/kallisto/Quants', '/', meta_data$Run)
...
txi_kallisto = tximport::tximport(files = paste0('./Outputs/kallisto/Quants', '/', meta_data$Run, '/', 'abundance.tsv'), type = 'kallisto', tx2gene = t2g, dropInfReps = T)
...
dds = DESeq2::DESeqDataSetFromTximport(txi = txi_kallisto, colData = meta_data, design = ~ Condition)
...
### Extract String IDs for significantly upregulated and significantly downregulated genes ###
